      ...
      <div v-else class="log-list">
        <div v-for="entry in logEntries" :key="entry.id" class="log-entry" :class="{ undone: entry.undone, 'new-entry': !entry.undone && !(Array.isArray(readIds) ? readIds : []).includes(entry.id) }">
          ...
          <div class="log-status">
            ...
            <template v-else>
              <button v-if="!readIds.includes(entry.id)" class="mark-read-btn" @click.stop="markEntryRead(entry)" title="Als gelesen">
...
async function runSuggestionsAnalysis() {
...
function markAllRead() {
  ...
  ids.forEach(id => { if (!readIds.value.includes(id)) readIds.value.push(id) })
